...
def event_settings(request, slug):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect("events:settings_event", slug=event.slug)
    else:
        form = EventSettingsForm(instance=event)
    return render(request, "settings/event_settings.html", {"event": event, "form": form})
...
def event_create(request):
    ...
    return render(request, "settings/event_create.html", {"form": form})
...
def trigger_sync(request, slug):
    ...
    if not event.sheets_enabled:
        ...
        return redirect("events:settings", slug=slug)
...
def edit_links(request, slug):
    ...
    if request.method == "POST":
        ...
        return redirect("events:settings_links", slug=slug)

    return render(request, "settings/links_edit.html", {"event": event, "links": event.links.all()})
...
def room_images(request, slug):
    ...
    return render(request, "settings/room_images.html", {"event": event, "rooms": rooms})
...
def upload_room_image(request, slug, room_id):
    ...
    if not file or not file.name:
        ...
        return redirect("events:settings_room_images", slug=slug)
...
def upload_pdf(request, slug):
    ...
    return redirect("events:settings_pdf_assign", slug=slug, batch_id=batch_id)
...
def assign_pdf_pages(request, slug, batch_id):
    ...
    return render(request, "settings/pdf_assign.html", {
        "event": event,
        "batch_id": batch_id,
        "pages": pages,
